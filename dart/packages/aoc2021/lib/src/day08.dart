...
class Day08 extends AdventDay {
  ...
  @override
  dynamic part1(String input) => input.lines.map(Entry.parse).map((entry) {
        final easy = {
  ...
  @override
  dynamic part2(String input) => input.lines.map(Entry.parse).map((entry) {
        ...
        codes[3] =
            d5.firstWhere((s) => codes[1].chars.every((c) => s.contains(c)));
        d5.remove(codes[3]);
        ...
        codes[6] =
            d6.firstWhere((s) => !codes[1].chars.every((c) => s.contains(c)));
        d6.remove(codes[6]);

        // The 9 contains all of the 4 segments.
        codes[9] =
            d6.firstWhere((s) => codes[4].chars.every((c) => s.contains(c)));
        d6.remove(codes[9]);

        // The left over six segment must be the 0.
        codes[0] = d6.first;

        final digits = Map.fromEntries(
            codes.asMap().entries.map((e) => MapEntry(e.value, e.key)));
